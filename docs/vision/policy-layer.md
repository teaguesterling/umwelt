...
Selectors in umwelt extend CSS's descendant combinator with a **context-qualifier** semantics for cross-taxon compounds. Within a taxon, `dir[name="src"] file[name$=".py"]` walks the plugin-declared parent/child hierarchy. Across taxa, `tool[name="Bash"] file[path^="src/auth/"]` reads as "when the acting tool is Bash, these files." The cascade target is always the rightmost entity; the context qualifier contributes specificity without changing cascade scope. This means one view can express actor-conditioned policy directly — different rules for different acting tools, different sub-delegate contexts, different job roles — without duplicating the target-entity selector for each actor. See [`entity-model.md`](./entity-model.md) §4.3 and §5.3 for the details.
